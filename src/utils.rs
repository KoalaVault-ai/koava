...
impl CryptoUtils {
    ...
    /// Returns Vec<u8> containing [8 bytes length prefix][JSON header]
    pub async fn read_safetensors_header_raw<P: AsRef<Path>>(file_path: P) -> Result<Vec<u8>> {
        ...
        // Allocate buffer for length prefix + JSON header
        let mut full_header_bytes = vec![0u8; Self::HEADER_LENGTH_SIZE + header_len];

        // Copy length prefix
        full_header_bytes[..Self::HEADER_LENGTH_SIZE].copy_from_slice(&header_len_bytes);

        // Read JSON directly into the rest of the buffer
    ...
    pub async fn extract_safetensors_header<P: AsRef<Path>>(file_path: P) -> Result<String> {
        let full_header_bytes = Self::read_safetensors_header_raw(file_path).await?;
        // Encode as base64
    ...
    pub async fn detect_safetensors_encryption<P: AsRef<Path>>(file_path: P) -> Result<bool> {
        ...
        // Skip header length prefix (8 bytes) to get JSON part
